...
advanced_questions = [
    ...
    {  # 29
        ...
        'options': ['Ensures thread safety in multithreaded programs', 'Implements a mutual exclusion mechanism', 'Facilitates inter-process communication',
...
def courseregpython(request):
    ...
    # Convert course fee to paise (Razorpay uses smallest currency unit)
    fee_in_paise = int(course.fee * 100)
    ...
    context = {
        ...
        'course.fee_in_paise': fee_in_paise
    }
...
def verify_payment(request):
    if request.method == "POST":
        ...
        # Create the data dictionary to verify signature
        params_dict = {
        ...
        try:
            ...
            course = CourseDB.objects.get(course_id=course_id)
            ...
            email_subject = f"Course : {course.course_name}"
            email_body = f"Congratulations!\n\nYou have successfully started the course: {course.course_name}.\n\nWe wish you the best of luck as you begin your learning journey!\n\nBest Regards,\nYour Learning Platform Team"
            
            # Send the email
            email = EmailMessage(
                email_subject, 
                email_body, 
                settings.EMAIL_HOST_USER,  # Your email address set in Django settings
                [request.user.email]  # Send to the authenticated user's email address
            )
            # Send the email
            email.send()
            payment.save()
            
            # Update the course registration with start and end dates
            ...
            end_date = start_date + timedelta(days=int(course.duration) * 7)  # Assuming duration is in weeks
